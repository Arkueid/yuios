...
int sys_execve(char *filename, char *argv[], char *envp[])
{
    ...
    for (size_t i = 0; i < ehdr->e_phnum; i++)
    {
        ...
        DEBUG("segment vaddr 0x%p paddr 0x%p\n", ptr->p_vaddr, ptr->p_paddr);
        ptr++;
    }
    ...
    for (size_t i = 0; i < ehdr->e_shnum; i++)
    {
        ...
        DEBUG("section %s size %d vaddr 0x%p\n",
    ...
    {
        DEBUG("v: 0x%X size: %d sec: %d b: %d t: %d n: %s\n",
             symptr->st_value,
             symptr->st_size,
             symptr->st_shndx,
